...
async def submit_transcription_task(file_url: str) -> Optional[str]:
    """
    ...
    async with httpx.AsyncClient(timeout=config.API_GENERAL_TIMEOUT) as client:
        response = await client.post(service_url, headers=headers, json=data)
...
async def transcribe_video(video_path: str, audio_path: str, file_serve_url: str) -> list[Segment]:
    ...
    task_id = await submit_transcription_task(file_serve_url)
    if not task_id:
