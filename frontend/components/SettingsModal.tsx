      ...
      <div className="bg-white border border-gray-200 rounded-2xl shadow-2xl w-full max-w-md overflow-hidden animate-in zoom-in-95 duration-200">
        ...
        <div className="p-8 space-y-6">
          <div className="space-y-2">
            <label className="text-xs font-bold uppercase tracking-wider text-gray-500">Transcription Model</label>
            <div className="relative">
                <select 
                value={localSettings.transcriptionModel}
                onChange={(e) => setLocalSettings({...localSettings, transcriptionModel: e.target.value})}
                ...
                >
                {MODELS_TEXT.map(m => <option key={m} value={m}>{m}</option>)}
                </select>
                <div className="pointer-events-none absolute inset-y-0 right-0 flex items-center px-4 text-gray-500">
                    <svg className="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M19 9l-7 7-7-7"></path></svg>
                </div>
            </div>
            <p className="text-[11px] text-gray-400 pl-1">Used for initial speech-to-text and speaker diarization.</p>
          </div>

          <div className="space-y-2">
            <label className="text-xs font-bold uppercase tracking-wider text-gray-500">Translation Model</label>
            <div className="relative">
                <select 
                value={localSettings.translationModel}
                onChange={(e) => setLocalSettings({...localSettings, translationModel: e.target.value})}
                className="w-full bg-white border border-gray-300 rounded-xl px-4 py-3 text-sm text-gray-800 focus:outline-none focus:border-claude-accent focus:ring-1 focus:ring-claude-accent transition appearance-none font-medium"
                >
                {MODELS_TEXT.map(m => <option key={m} value={m}>{m}</option>)}
                </select>
            ...
            <p className="text-[11px] text-gray-400 pl-1">Used for translating text segments into target language.</p>
          </div>

          <div className="space-y-2">
            <label className="text-xs font-bold uppercase tracking-wider text-gray-500">TTS Model</label>
            <div className="relative">
                <select 
                ...
                className="w-full bg-white border border-gray-300 rounded-xl px-4 py-3 text-sm text-gray-800 focus:outline-none focus:border-claude-accent focus:ring-1 focus:ring-claude-accent transition appearance-none font-medium"
                >
                {MODELS_TTS.map(m => <option key={m} value={m}>{m}</option>)}
                </select>
                <div className="pointer-events-none absolute inset-y-0 right-0 flex items-center px-4 text-gray-500">
                    <svg className="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M19 9l-7 7-7-7"></path></svg>
                </div>
            </div>
            <p className="text-[11px] text-gray-400 pl-1">Used for synthesizing voice audio from translated text.</p>
          </div>
        </div>
